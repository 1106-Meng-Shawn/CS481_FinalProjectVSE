...
public class PreviewCharacters : MonoBehaviour
{
    ...
    public static PreviewCharacters Instance { get; private set; }
    ...
    public PreviewCharacter GetRandomPreviewCharacter()
    {
        int maxSize = GetPreviewCharacterLength();
        int playerIndex = UnityEngine.Random.Range(0, maxSize);
        PreviewCharacter Char = GetPreviewCharacter(playerIndex);
        return Char;
    }


    public List<PreviewCharacter> GetAllPreviewCharacters()
    {
        List<PreviewCharacter > list = new List<PreviewCharacter>();
        foreach (var character in _characters)
        {
            list.Add(character.GetComponent<PreviewCharacter>());
        }

        return list;
    }

    public PreviewCharacter GetPreviewCharacter(int index)
    {
        return _characters[index].GetComponent<PreviewCharacter>();
    }

    public int GetPreviewCharacterLength()
    {
